...
In practice, PPO/GRPO are best understood as **stochastic gradient ascent (SGA)** methods that compute a clipped gradient estimator. [Li et al., 2025](https://richardli.xyz/rl-collapse-1) analyze how mismatch between sampling policy {{< math >}}$\mu${{< /math >}} and target policy {{< math >}}$\pi${{< /math >}} affects optimization. The token-level importance sampling gradient used in PPO/GRPO is essentially the gradient of the surrogate objective {{< math >}}$\nabla_\theta L_\mu${{< /math >}}, which introduces bias relative to the true gradient {{< math >}}$\nabla_\theta J${{< /math >}} that scales with both horizon and policy divergence.
