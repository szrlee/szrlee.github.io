...
- **Forward reference**: See Assumption A3 below for the stronger condition {{< math >}}$I(G; \xi) = O(1)${{< /math >}}
...
Also (since mutual information is non-negative):
{{< math >}}
$$I(X; Y, Z) = I(X; Z) + I(X; Y | Z) \geq I(X; Z) \quad \text{(because } I(X; Y | Z) \geq 0\text{)}$$
{{< /math >}}
...
**Why this proof works**: The key insight is that {{< math >}}$\pi^*${{< /math >}} is a *deterministic function* of {{< math >}}$\xi${{< /math >}} (by Assumption A1). This deterministic relationship allows us to use the chain rule for mutual information to relate {{< math >}}$I(G; \pi^*)${{< /math >}} and {{< math >}}$I(G; \xi)${{< /math >}}. We don't need the full Markov chain structure {{< math >}}$\xi \to \pi^* \to G${{< /math >}} for this argument—the chain rule alone is sufficient.
...
{{< math >}}
$$I(\delta_t; \pi^* | \mathcal{D}_{\text{past}}) = I(\delta_t; \pi^*_\xi | \mathcal{D}_{\text{past}}) \leq I(\delta_t; \xi | \mathcal{D}_{\text{past}})$$
{{< /math >}}

where {{< math >}}$\mathcal{D}_{\text{past}} = \{\delta_0, \ldots, \delta_{t-1}\}${{< /math >}} denotes all previously observed TD errors in the current episode.
...
{{< math >}}
$$H(\delta_t | \mathcal{D}_{\text{past}}) \leq \log_2(B_\delta) = O(1)$$
{{< /math >}}

Since we have {{< math >}}$T${{< /math >}} steps (one per token generated):

{{< math >}}
$$\mathcal{B}_{\text{potential}} = \sum_{t=0}^{T-1} H(\delta_t | \mathcal{D}_{\text{past}}) = T \cdot O(1) = O(T) \text{ bits}$$
{{< /math >}}
...
**Step 4**: Analyze {{< math >}}$I(\delta_t; \xi | \mathcal{D}_{\text{past}})${{< /math >}} — the information content of TD errors.
...
{{< math >}}
$$I(\delta_T; \xi | s_T, \mathcal{D}_{\text{past}}) = O(1) \text{ bits}$$
{{< /math >}}
...
Therefore:
{{< math >}}
$$I(\delta_t; \xi | s_t, s_{t+1}, \mathcal{D}_{\text{past}}) \leq I(V_\phi; \xi) \leq O(M)$$
{{< /math >}}
...
**Define the correlation factor** {{< math >}}$\alpha \in (0, 1]${{< /math >}} **as the effective information density**:

{{< math >}}
$$\alpha \approx \frac{I(\{\delta_t\}_{t=0}^{T-1}; \xi)}{T \cdot I(\delta_0; \xi)}$$
{{< /math >}}

This measures how much information the entire sequence of TD errors provides, normalized by the naïve upper bound ({{< math >}}$T${{< /math >}} independent signals each carrying {{< math >}}$I(\delta_0; \xi)${{< /math >}} bits).
...
- LoRA trainable parameters: {{< math >}}$2 \times 8 \times 4096 = 65{,}536${{< /math >}}
- LoRA storage capacity (32-bit floats): {{< math >}}$32 \times 65{,}536 \approx 2{,}097{,}152${{< /math >}} bits
- Information accumulated: {{< math >}}$\approx 2{,}000${{< /math >}} bits
- **Storage capacity ratio**: {{< math >}}$\sim 1000\times${{< /math >}} more storage than information
- **Degrees of freedom ratio**: {{< math >}}$\sim 30\times${{< /math >}} more parameters than episodes

**What this suggests** (not proves):

1. Even comparing degrees of freedom (65,536 parameters vs {{< math >}}$\sim 1{,}000${{< /math >}} training episodes), LoRA provides substantial capacity
2. Comparing storage capacity (2M bits vs 2K information bits), the overprovisioning is dramatic
3. This explains why LoRA rarely hits a capacity bottleneck in practice

**What we CANNOT claim rigorously**:

- We cannot equate "parameter count" with "information capacity" quantitatively
- We cannot equate "storage bits" (representation) with "information bits" (uncertainty reduction)
...
**For different algorithms** (information-theoretic lower bound on sample complexity):
...
**Theoretical speedup** (best-case upper bound):
{{< math >}}
...
{{% callout warning %}}
**Caveat**: This example assumes a discretized or effectively bounded policy space where {{< math >}}$H(\pi^*) \approx 10{,}000${{< /math >}} bits. For continuous unbounded parameter spaces, differential entropy {{< math >}}$h(\pi^*)${{< /math >}} may be infinite, but the mutual information {{< math >}}$I(S; \pi^*)${{< /math >}} remains finite and meaningful. The specific value of 10,000 bits is chosen for illustration—actual policy entropy depends on prior strength and problem structure.
{{% /callout %}}
